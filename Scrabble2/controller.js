document.getElementById("info").classList.remove("transparent");
document.getElementById("info").innerHTML += "BEGIN controller.js";
const MAX_REDRAW = 10;
...
class Controller {
    ...
    constructor() {
		document.getElementById("info").classList.remove("transparent");
        document.getElementById("info").innerHTML += "CONTROLLER CONSTRUCTOR";
		
        this.model = new Model();
		document.getElementById("info").innerHTML += "MODEL CREATED";
        this.drawTiles();
		document.getElementById("info").innerHTML += "DRAWTILES";
        this.refreshScorePanel();
		document.getElementById("info").innerHTML += "SCORE REFRESHED";
    }
    ...
    isValidWordWithOrWithoutEscapingWhiteSpaces(word, escape) {
        if(escape == true)
            word = word.replaceAll(' ', '');

        let position = word.search(' ');
        if(position > -1) {
            let words = [word];
            let positions = [];
            while(position > -1) {
                ...
                for(const l of "ABCDEFGHIJKLMNOPQRSTUVWXYZ") {
                    for(const w of words) {
                        let str = w.replace(/ /, "?" + l);
                        if(end == true) {
                            if(dictionary.has(str.replaceAll("?", "").toUpperCase())) {
                                words2.push({word: str.replaceAll("?", " "), whites: positions});
                            }
                        } else
                            words2.push(str);
                    }
    ...
    isValidWord(word) {
        return this.isValidWordWithOrWithoutEscapingWhiteSpaces(word, true);
    }
