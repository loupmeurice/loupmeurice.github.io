        ...
        if(position > -1) {
            ...
            while(position > -1) {
                ...
                for(const l of "ABCDEFGHIJKLMNOPQRSTUVWXYZ") {
					let iterator = words.length ? words : words.unexistingWords;
                    for(const w of iterator) {
                        let str = w.replace(/ /, "?" + l);
                        if(end == true) {
                            if(dictionary.has(str.replaceAll("?", "").toUpperCase())) {
								existingWords.push(str.replaceAll("?", " "));
                            } else {
								unexistingWords.push(str.replaceAll("?", " "));	
							}
                        } else
                            unexistingWords.push(str);
                    }
                }

                words = {existingWords: [...existingWords], unexistingWords: [...unexistingWords]};
